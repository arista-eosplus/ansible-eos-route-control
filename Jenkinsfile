...
/*
 * Jenkinsfile for ansible-eos-route-control role
 *
 * Run the Ansible-Role-Test job against a commit or
 * pull request in the ansible-eos-route-control repo
 */
...
pipeline {
    ...
    environment {
        projectName = 'ansible-eos-route-control'
        emailTo = 'ansible-dev+jenkins@arista.com'
        emailFrom = 'eosplus-dev@arista.com'
    }

    stages {
        stage ('Run tests for ansible-eos-route-control role') {
            when {
            ...
            steps {
                ...
                build job: 'Ansible-Role-Test-Starter',
                      parameters: [
                          string(name: 'ROLE_NAME', value: 'ansible-eos-route-control'),
